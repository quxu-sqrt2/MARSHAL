...
class AgenticConfig(BaseConfig):
    ...
    memory_enabled: bool = field(
        default=False,
        metadata={"help": "Enable social memory retrieval and logging."}
    )
    memory_db_path: Optional[str] = field(
        default=None,
        metadata={"help": "SQLite path for the social memory bank. Defaults to output_dir/memory/social_memory.sqlite3."}
    )
    memory_top_k1: int = field(
        default=50,
        metadata={"help": "Phase-A retrieval top-k by cosine similarity."}
    )
    memory_top_k2: int = field(
        default=5,
        metadata={"help": "Phase-B retrieval top-k by Q-value re-ranking."}
    )
    memory_alpha: float = field(
        default=0.1,
        metadata={"help": "Learning rate for Q-value updates in social memory bank."}
    )
    memory_embedding_dim: int = field(
        default=128,
        metadata={"help": "Embedding dimension for mock intent encoder."}
    )
    memory_intent_turns: int = field(
        default=3,
        metadata={"help": "How many recent turns to build the social intent string."}
    )
