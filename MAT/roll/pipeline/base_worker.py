...
class ActorWorker(Worker):
    ...
    def initialize(self, pipeline_config):
        ...
        if getattr(self.strategy, "uses_cuda", True):
    ...
    def train_step(self, data: DataProto):
        ...
        """
        if getattr(self.pipeline_config, "freeze_model", False):
            self.logger.info(f"{self.worker_name} freeze_model enabled, skip train_step")
            return DataProto(meta_info={"metrics": {"actor/train_skipped": 1}})
