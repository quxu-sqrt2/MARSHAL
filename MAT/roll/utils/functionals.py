...
def compute_advantage(
    ...
    if adv_estimator == "memrl":
        response_level_rewards = data.batch.get("response_level_rewards")
        if response_level_rewards is None:
            response_level_rewards = data.batch["token_level_rewards"].sum(dim=-1)

        if advantage_norm:
            response_level_rewards = normalize_unique_values_by_player(
                response_level_rewards, data, mode=advantage_norm
            )

        returns = response_level_rewards.unsqueeze(1).repeat(1, response_mask.shape[1])
        ...
        data.batch["returns"] = returns * response_mask
        return data
